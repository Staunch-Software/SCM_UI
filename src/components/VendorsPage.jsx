...
const VendorsPage = () => {
  ...
  const fetchSuppliers = async () => {
    try {
      const response = await apiClient.get("/api/vendors/suppliers-list");
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await apiClient.post("/api/vendors/create", formData);
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        setFormData({
          ...
          state_id: "", zip: "", country_id: "", x_studio_supplier_since: ""
        });
  ...
  const handleSupplierClick = (supplierName) => {
    // We no longer check if the vendor exists in the metrics list.
    // We navigate unconditionally to provide a better user experience.
    // The VendorMetricsPage will handle showing a "not found" message.
  ...
  return (
    <div className="vendors-page">
      ...
      {/* --- MODAL AND POPUP CODE (UNCHANGED) --- */}
      {showModal && (
