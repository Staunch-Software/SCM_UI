...
const PurchaseOrderDrawer = ({ isOpen, onClose, orderId }) => {
  ...
  const handleCreateOrder = async () => {
    if (!editedHeader.supplier_id) {
      setHeaderError("Please select a supplier");
      return;
    }
    if (!editedHeader.expected_arrival_date) {
      setHeaderError("Please select expected arrival date");
      return;
    }
      ...
      <div className={`drawer-container ${isOpen ? 'open' : ''}`}>
        <div className="drawer-header">
          <h2 className="drawer-title">
            {orderId ? `Purchase Order ${orderId}` : 'New Purchase Order'}
          </h2>
                      ...
                      {orderData?.line_items && orderData.line_items.length > 0 ? (
                        ...
                        <tr>
                          <td colSpan="6" style={{ textAlign: 'center', padding: '2rem' }}>
                            {!orderId ? "Create the order first to add line items" : "No line items found"}
                          </td>
                        </tr>
                      )}
                    </tbody>
        ...
        <div className="drawer-footer">
          ...
          {isEditable && (
            <button
              ...
              onClick={orderId ? handleUpdateHeader : handleCreateOrder}
              disabled={updatingHeader}
            >
              {updatingHeader ? "Saving..." : (orderId ? "Update" : "Create")}
            </button>
          )}
        </div>
      </div>
